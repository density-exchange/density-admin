...
export default function RewardScreen() {
	...
	const getUserData = () => {
		if (!email) return;
		makeGetReq(`/v1/users/email?email=${email}`)
			.then((res) => {
				setUserData(res);
				setGo(true);
			})
				...
				<Box display="flex" className="reward_user_card">
					<Box className="enterEmailSection">
						<Box mt={2} display="flex" gap="0.5rem">
							...
							<Button onClick={getUserData} sx={{ height: '51px' }} variant="outlined">
								<Typography variant="h4">Go</Typography>
							</Button>
						</Box>
						<Box mt={3} className="rewardButton">
							{go && (
								<Button onClick={toggleRewardFormModal} fullWidth variant="contained">
					...
					<Stack spacing={2}>
						<Box sx={{ minWidth: 120 }}>
							<FormControl fullWidth>
								<InputLabel id="demo-simple-select-label">Select the reward type</InputLabel>
								<Select
						...
						<TextField
							label="Enter the reward amount"
							onChange={(e) => setRewardData({ ...rewardData, amount: e.target.value })}
						/>
						<TextField
							label="Enter the comments"
							onChange={(e) => setRewardData({ ...rewardData, remarks: e.target.value })}
						/>
